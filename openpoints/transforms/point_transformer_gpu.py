...
@DataTransforms.register_module()
class PointCloudCenterAndNormalizeWithExtrinsic(object):
    def __init__(
        self,
        centering=True,
        normalize=True,
    ...
    def __call__(self, data):
        if hasattr(data, "keys"):
            if self.append_xyz:
                ...
                data["heights"] = height - torch.min(height)
            # 一定有转换
            centroid = torch.mean(data["pos"], axis=0, keepdims=True)
            ...
            S_trans = torch.eye(4).to(data["pos"].device)
            S_trans[:3, 3] = -centroid
            S_rot = torch.eye(4).to(data["pos"].device)
            S_rot[:3, :3] /= m
            S = S_rot @ S_trans
...
@DataTransforms.register_module()
class PointCloudScaleAndJitterWithExtrinsic(object):
    def __init__(
        self,
        scale=[2.0 / 3, 3.0 / 2],
        scale_xyz=[True, True, True],  # ratio for xyz dimenions
        anisotropic=False,  # scaling in different ratios for x, y, z
        jitter_sigma=0.01,
        jitter_clip=0.05,
        mirror=[0, 0, 0],  # mirror scaling, x --> -x
        **kwargs
    ...
    def __call__(self, data):
        device = data["pos"].device if hasattr(data, "keys") else data.device
        if self.anisotropic:
            random_scale = torch.rand(3, dtype=torch.float32, device=device)
        else:
            random_scale = torch.rand(1, dtype=torch.float32, device=device).repeat(3)

        scale = random_scale * (self.scale_max - self.scale_min) + self.scale_min
        ...
        if hasattr(data, "keys"):
            ...
            S = torch.eye(4).to(data["pos"].device)
            S[:3, :3] *= scale
            S = torch.linalg.inv(S)
            for key in self.keys:
                assert key in data.keys()
...
@DataTransforms.register_module()
class PointCloudRotationWithExtrinsic(object):
    def __init__(self, angle=[0.01, 0.01, 2], **kwargs):
        self.angle = np.array(angle) * np.pi
        self.keys = ["extrinsic"]
