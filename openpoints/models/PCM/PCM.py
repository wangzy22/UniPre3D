...
from .PointMLP_layers import (
    ConvBNReLU1D,
    LocalGrouper,
    PreExtraction,
    PreExtraction_Replace,
    PosExtraction,
    get_activation,
...
from fusion.feat_fusion import FeatureFusion
...
class PointMambaEncoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if not isinstance(mamba_layers_orders, list):
            mamba_layers_orders = [
                mamba_layers_orders for i in range(sum(mamba_blocks))
            ]
        else:
        ...
        if use_order_prompt:
            ...
            order2idx = {
                order: [i * prompt_num_per_order, (i + 1) * prompt_num_per_order]
        ...
        if self.mamba_pos:
            if self.pos_type == "share":
                if sum(dim_expansion) == len(dim_expansion):
                    ...
                    self.pos_proj = self.pos_proj_type(
                        3, embed_dim * dim_expansion[0], bias=False
                    )
        ...
        dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(mamba_blocks))
        ...
        for i in range(len(pre_blocks)):
            ...
            local_grouper = LocalGrouper(
                last_channel, reduce, kneighbor, use_xyz, normalize, k_stride=kstride
            )  # [b,g,k,d]
            self.local_grouper_list.append(local_grouper)
            # append pre_block_list
            if pre_block_num == 0:
                # only max pooling
                pre_block_module = PreExtraction_Replace(
                    last_channel,
                    out_channel,
                    pre_block_num,
                    groups=groups,
            ...
            if last_channel == out_channel or i == 0 or not self.block_residual:
                ...
                self.residual_proj_blocks_list.append(
                    nn.Linear(last_channel, out_channel, bias=False)
                )
    ...
    def forward_cls_feat(self, p, x=None):
        ...
        for i in range(self.stages):
            ...
            # GAM forward
            p, x, x_res = self.local_grouper_list[i](
                p, x.permute(0, 2, 1), x_res
            )  # [b,g,3]  [b,g,k,d]
            ...
            for layer in self.mamba_blocks_list[i]:
                p, x, x_res = self.serialization_func(
                    p, x, x_res, self.mamba_layers_orders[mamba_layer_idx]
                )
                if self.use_windows:
                ...
                if self.use_order_prompt:
                    ...
                    layer_order_prompt = layer_order_prompt.unsqueeze(0).repeat(
                        p.shape[0], 1, 1
                    )
                    x = torch.cat([layer_order_prompt, x, layer_order_prompt], dim=1)
                    if x_res is not None:
                        x_res = torch.cat(
                            [layer_order_prompt, x_res, layer_order_prompt], dim=1
                        )
                    x, x_res = layer(x, x_res)
                    x = x[:, self.promot_num_per_order : -self.promot_num_per_order]
                    x_res = x_res[
                        :, self.promot_num_per_order : -self.promot_num_per_order
                    ]
                else:
                    x, x_res = layer(x, x_res)
                if self.use_windows:
                    p, x, x_res = self.post_split_windows(
                        p, x, x_res, n_windows, p_base
                    )
                mamba_layer_idx += 1
            x = x.permute(0, 2, 1).contiguous()
    ...
    def forward_seg_feat(self, p, x=None):
        ...
        for i in range(self.stages):
            ...
            for layer in self.mamba_blocks_list[i]:
                ...
                if self.use_windows:
                    p, x, x_res = self.post_split_windows(
                        p, x, x_res, n_windows, p_base, p_std
                    )
                mamba_layer_idx += 1
            x = x.permute(0, 2, 1).contiguous()
            x = self.pos_blocks_list[i](x)  # [b,d,g]
    ...
    def pre_split_windows(self, p, x, x_res, windows_size=1024):
        ...
        if new_x_res is not None:
            new_x_res = (
                new_x_res.reshape(bs, n_splits, windows_size, -1)
                .flatten(0, 1)
                .contiguous()
            )
...
class PointMambaDecoder(nn.Module):
    """Point Cloud Mamba Decoder for feature processing and fusion.

    This decoder combines PointNet-style feature propagation with Mamba blocks
    for sequence modeling and optional image feature fusion.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """Initialize the PointMambaDecoder.

        Args:
            encoder_channel_list (List[int]): Channel dimensions for encoder features
            decoder_channel_list (List[int]): Channel dimensions for decoder layers
            decoder_blocks (List[int]): Number of blocks in each decoder layer
            mamba_blocks (List[int]): Number of Mamba blocks in each layer
            mamba_layers_orders (List[str]): Order of operations for each Mamba layer
            act_args (str): Activation function type
            rms_norm (bool): Whether to use RMS normalization
            fused_add_norm (bool): Whether to use fused add norm optimization
            residual_in_fp32 (bool): Whether to compute residuals in FP32
            bimamba_type (str): Type of BiMamba architecture
            gmp_dim (int): Global max pooling feature dimension
            bias (bool): Whether to use bias in layers
        """
        super().__init__()

        # Reverse encoder dimensions for decoder construction
        ...
        # Prepare dimensions for decoder construction
        en_dims.reverse()
        ...
        # Validate dimensions
        assert (
            len(en_dims) == len(de_dims) == len(de_blocks) + 1 == len(mamba_blocks) + 1
        ), "Dimension mismatch in decoder configuration"
        assert (
            sum(mamba_blocks) == len(mamba_layers_orders) or sum(mamba_blocks) == 0
        ), "Mamba blocks and layer orders mismatch"

        # Build decoder layers with Mamba blocks
        mamba_layer_idx = 0
        for i in range(len(en_dims) - 1):
            # Add feature propagation layer
            self.decode_list.append(
                PointNetFeaturePropagation(
                    de_dims[i] + en_dims[i + 1],  # Input channels
                    de_dims[i + 1],  # Output channels
                    blocks=de_blocks[i],
                    res_expansion=1.0,
                    bias=True,
                    activation=act_args,
            ...
            # Add Mamba blocks for this layer
        ...
        # Setup global max pooling feature mapping
        self.gmp_map_list = nn.ModuleList()
        for en_dim in en_dims:
            self.gmp_map_list.append(
                ConvBNReLU1D(en_dim, gmp_dim, bias=bias, activation=act_args)
            )
        ...
        # Configure feature fusion
        self.use_fusion = kwargs.get("use_fusion", True)

    def serialize_func(self, p, x, x_res, order, layers_outputs=[]):
        """Serialize point cloud data based on specified order.

        Args:
            p: Point coordinates
            x: Features
            x_res: Residual features
            order: Serialization order
            layers_outputs: List to store layer outputs

        Returns:
            tuple: Processed points, features, and residuals
        """
        if order == self.order or order == "null":
            return p, x, x_res
        else:
            p, x, x_res = serialization(
                p, x, x_res=x_res, order=order, layers_outputs=layers_outputs
            )
            self.order = order
            return p, x, x_res

    def forward(
        self, p, f, image_features, c2w_projection_matrix, feature_mlps, intrinsic
    ):
        """Forward pass of the decoder.

        Args:
            p: List of point coordinates at different scales
            f: List of point features at different scales
            image_features: Image features for fusion
            c2w_projection_matrix: Camera-to-world projection matrix
            feature_mlps: MLPs for feature processing
            intricsic: Camera intrinsic parameters
        ...
        # Prepare point and feature lists for decoding
        xyz_list = p
        x_list = f
        xyz_list.reverse()
        x_list.reverse()
        ...
        fusion_list = [len(self.decode_list) - 1] if self.use_fusion else []
