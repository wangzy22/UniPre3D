...
class BaseSeg(nn.Module):
    def __init__(self, encoder_args=None, decoder_args=None, cls_args=None, **kwargs):
        ...
        if decoder_args is not None:
            ...
            decoder_args_merged_with_encoder["encoder_channel_list"] = (
                ...
                and not decoder_args_merged_with_encoder["encoder_channel_list"]
                else decoder_args["encoder_channel_list"]
            )
        ...
        if cls_args is not None:
            if cls_args.get("in_channels", None):
                in_channels = cls_args.get("in_channels", None)
            elif hasattr(self.decoder, "out_channels"):
    ...
    def forward(
        ...
        p, f = self.encoder.forward_seg_feat(pts)
        if self.decoder is not None:
            f = self.decoder.forward(
                p, f, image_features,
        c2w_projection_matrix,
        feature_mlps,
        intricsic,
...
class BasePartSeg(BaseSeg):
    ...
    def forward(self, p0, f0=None, cls0=None):
        cls0 = torch.zeros((p0.shape[0], p0.shape[1]))
        if hasattr(p0, "keys"):
