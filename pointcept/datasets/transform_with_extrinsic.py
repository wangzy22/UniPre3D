...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        for name, keys in self.kwargs.items():
            ...
            data[name] = torch.cat([data_dict[key].float() for key in keys], dim=1) if len(keys) > 1 else data_dict[keys[0]].float()
        return data
...
class NormalizeColor(object):
    ...
    def __call__(self, data_dict):
        ...
        if "gt_images" in data_dict.keys():
            if self.normalization_type == "gaussian":
                ...
                data_dict["gt_images"] = np.array(
                    IMAGE_TRANSFORMS(
                        torch.from_numpy(data_dict["gt_images"] / 255.0).clip(0, 1)
                    )
                )
                ######################
            else:
                data_dict["gt_images"] = (data_dict["gt_images"] / 255.0).clip(0, 1)
        return data_dict
...
@TRANSFORMS.register_module()
class FPS(object):
    def __init__(self, max_points=80000, return_inverse=True):
        """
    ...
    def __call__(self, data_dict):
        """
        Apply furthest point sampling (FPS) to each scene in the point cloud data.

        Args:
            data_dict (dict): Contains point cloud data with 'coord', 'offset', and other optional keys (e.g., 'color', 'normal').

        Returns:
            data_dict (dict): The updated dictionary with downsampled point clouds.
        """
        coord = data_dict["coord"]
        offset = data_dict["offset"]
        N = coord.shape[0]  # Total number of points across all scenes

        now_index = 0  # Current index in the stacked point cloud
        idx_list = []  # To store indices of sampled points
        inverse = torch.full(
            (N,), -1, dtype=torch.long
        )  # Initialize inverse with -1 (invalid index)
        new_offset = []
        # Loop over each scene
        for i in range(len(offset)):
            # Extract the coordinates for the current scene
            start_idx = now_index
            end_idx = now_index + offset[i]
            scene_coords = coord[start_idx:end_idx].cuda()
            if scene_coords.shape[0] > self.max_points:
                # Apply furthest point sampling for the current scene
                idx = (
                    furthest_point_sample(scene_coords.unsqueeze(0), self.max_points)
                    .long()
                    .squeeze(0)
                    .cpu()
                )
            else:
                idx = torch.range(0, scene_coords.shape[0] - 1).long()
            # Adjust indices relative to the entire point cloud
            idx_list.append(idx + now_index)

            # Update inverse for the selected points
            inverse[idx + now_index] = torch.arange(len(idx)) + now_index

            # Update now_index for the next scene
            now_index += offset[i]

            # Store the number of points sampled in this scene for the new offset
            new_offset.append(len(idx))

        # Combine the indices from all scenes
        sampled_idx = torch.cat(idx_list)

        # Update the point cloud data with the sampled points
        for key in ["feat", "grid_coord", "coord", "strength", "segment", "instance"]:
            if key in data_dict:
                data_dict[key] = data_dict[key][sampled_idx]

        # Update inverse in data_dict
        if self.return_inverse:
            data_dict["fps_inverse"] = inverse

        # Update offset in data_dict with new offsets
        data_dict["offset"] = torch.tensor(new_offset, dtype=torch.long)
        return data_dict
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            ...
            if "gt_images" in data_dict.keys():
                contrast_feat = (data_dict["gt_images"][..., :3] - lo) * scale
                data_dict["gt_images"][..., :3] = (1 - blend_factor) * data_dict[
                    "gt_images"
...
class ChromaticTranslation(object):
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and np.random.rand() < self.p:
            ...
            if "gt_images" in data_dict.keys():
                data_dict["gt_images"][..., :3] = np.clip(
                    tr + data_dict["gt_images"][..., :3], 0, 255
                )
        return data_dict
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        if self.min_coord is not None:
            ...
            grid_coord = np.floor(
                (data_dict["coord"] - (min_coord)) / np.array(self.grid_size)
            ).astype(int)
        else:
        ...
        if self.mode == "train":  # train mode
            ...
            if self.return_inverse:
                if "inverse" in data_dict.keys():
                    # 根据已有 inverse 计算新的 inverse，包含嵌套
                    data_dict["inverse"] = data_dict["inverse"][idx_unique]
                    print(data_dict["inverse"].shape)
                else:
